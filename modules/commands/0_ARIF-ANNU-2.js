...
async function makeImage({ one, two }) {
    ...
    let batgiam_img = await jimp.read(__root + "/arm.png");
    ...
    batgiam_img.composite(circleOne.resize(230,230), 108, 255).composite(circleTwo.resize(230, 230), 434, 255);
...
module.exports.run = async function ({ event, api, args }) {    
    ...
    if (!mention[0]) return api.sendMessage("Please mention 1 person.", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "❥︎|====『  𝗟❤︎𝗩𝗲  』====|☻︎", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
      }